async function transform(input, xslt, inputType, resultsSelect) {
  ...
  if (saxonInitialized) {
	  try {
		  ...
		  if (contextItem === null) {
			var xpath = `let $result := transform(map { 'stylesheet-text' : $xslt, 'delivery-format' : 'serialized', 'base-output-uri' : 'files:///files/output' })
			...
			var xpath = `let $result := transform(map { 'stylesheet-text' : $xslt, 'delivery-format' : 'serialized', 'base-output-uri' : 'files:///files/output', 'initial-match-selection' : $context-item, 'global-context-item' : $context-item })
			             return map:for-each($result, function($uri, $serializedResult) { map { 'uri' : $uri, 'result' : $serializedResult } })`;
		  }
